...
public class Network extends JFrame {
    ...
    // Interaction Modes
    private enum Mode { ADD_VERTEX, ADD_CONNECTION, FIND_PATH }
    ...
    private void computeShortestPath(Vertex source, Vertex dest) {
        ...
        while (!pq.isEmpty()) {
            ...
            int u = vd.vertexId;
            if (vd.distance > dist[u]) continue;
            for (Connection conn : connections) {
                int v = -1;
                if (conn.from.id == u) v = conn.to.id;
                else if (conn.to.id == u) v = conn.from.id;
                if (v != -1) {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (Vertex v : vertices) {
                ...
                if (currentMode == Mode.FIND_PATH && (v == pathSource || v == pathDest)) {
                    ...
                    g2.setStroke(new BasicStroke(2));
                    g2.drawOval(v.x - Vertex.DIAMETER/2 - 3, v.y - Vertex.DIAMETER/2 - 3, Vertex.DIAMETER + 6, Vertex.DIAMETER + 6);
                }
            }
            ...
            if (currentMode == Mode.ADD_CONNECTION && tempVertex != null) {
                ...
                g2.setStroke(new BasicStroke(2));
                g2.drawOval(tempVertex.x - Vertex.DIAMETER/2 - 3, tempVertex.y - Vertex.DIAMETER/2 - 3, Vertex.DIAMETER + 6, Vertex.DIAMETER + 6);
            }
        }
            ...
            if (currentMode == Mode.ADD_VERTEX) {
                ...
                if (clicked != null) {
                    if (tempVertex == null) {
                        ...
                        String input = JOptionPane.showInputDialog(Network.this, "Enter cost and bandwidth (comma separated):", "Connection Details", JOptionPane.PLAIN_MESSAGE);
                        if (input != null && !input.trim().isEmpty()) {
                            try {
                                ...
                                JOptionPane.showMessageDialog(Network.this, "Invalid input. Please enter numeric cost and bandwidth.");
                            }
                        }
                        tempVertex = null;
                        repaint();
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> new Network().setVisible(true));
    }
}
